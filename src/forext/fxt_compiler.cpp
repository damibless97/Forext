...
void Compiler::declaration() {
  if (match(TokenType::TOKEN_CONTRACT)) {
    contractDeclaration();
  } else if (match(TokenType::TOKEN_FUNCTION)) {
    functionDeclaration();
  // } else if (match(TokenType::TOKEN_U256)) {
  //   varDeclaration();
  } 
  else if (match(TokenType::TOKEN_U256)) {
    varDeclaration();
...
void Compiler::function(FunctionObject::FunctionType type) {
  ...
  auto function = endCompiler();
  emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_CLOSURE),
...
void Compiler::varDeclaration() {
  ...
  if (match(TokenType::TOKEN_EQUAL)) {
    ...
    emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_NULL));
  }
...
void Compiler::contractDeclaration() {
  consume(TokenType::TOKEN_IDENTIFIER, "Expect contract name.");
  Token contractName = previous;
  uint8_t nameConstant = identifierConstant(previous);
  declareVariable();
  emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_CONTRACT),
            nameConstant);

  defineVariable(nameConstant);

  ContractEnvironment contractEnvironment;
  contractEnvironment.name = previous;
  contractEnvironment.hasSupercontract = false;
  contractEnvironment.enclosing = currentContractEnvironment;
  currentContractEnvironment = &contractEnvironment;
  if (match(TokenType::TOKEN_EXTENDS)) {
    consume(TokenType::TOKEN_IDENTIFIER, "Expect supercontract name.");
    variable(false);
    if (identifiersEqual(contractName, previous)) {
      reporter.error(previous, "A contract cannot inherit from itself.");
    }
    ...
    defineVariable(0);
    namedVariable(contractName, false);
    emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_INHERIT));
    contractEnvironment.hasSupercontract = true;
  }

  // Load the class onto the stack before compiling
  // its body, so that its methods can be bound to it.
  namedVariable(contractName, false);
  consume(TokenType::TOKEN_LEFT_BRACE, "Expect '{' before contract body.");
  while (!check(TokenType::TOKEN_RIGHT_BRACE) && !check(TokenType::TOKEN_EOF)) {
    method();
  }

  consume(TokenType::TOKEN_RIGHT_BRACE, "Expect '}' after contract body.");
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
  if (contractEnvironment.hasSupercontract) {
    endScope();
  }

  currentContractEnvironment = currentContractEnvironment->enclosing;
}
...
void Compiler::method() {
  ...
  function(type);
  emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_METHOD),
            constant);
}
...
void Compiler::ifStatement() {
  ...
  int thenJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP_IF_FALSE));
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
  statement();
  int elseJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP));
  patchJump(thenJump);
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
...
void Compiler::whileStatement() {
  int loopStart = currentBytecode()->getBytecodeCount();
  consume(TokenType::TOKEN_LEFT_PAREN, "Expect '(' after 'while'.");
  expression();
  consume(TokenType::TOKEN_RIGHT_PAREN, "Expect ')' after condition.");
  int exitJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP_IF_FALSE));
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
  statement();
  emitLoop(loopStart);
  patchJump(exitJump);
...
void Compiler::forStatement() {
  ...
  if (match(TokenType::TOKEN_SEMICOLON)) {
    // No initializer.
  } else if (match(TokenType::TOKEN_INTEGER)) {
    varDeclaration();
  } else {
    expressionStatement();
  }
  ...
  if (!match(TokenType::TOKEN_SEMICOLON)) {
    ...
    exitJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP_IF_FALSE));
    emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
  }

  if (!match(TokenType::TOKEN_RIGHT_PAREN)) {
    int bodyJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP));
    int incrementStart = currentBytecode()->getBytecodeCount();
    expression();
    emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
...
void Compiler::returnStatement() {
  ...
  if (match(TokenType::TOKEN_SEMICOLON)) {
    ...
    emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_RETURN));
  }
}

void Compiler::emitLoop(int loopStart) {
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_LOOP));
  int offset = currentBytecode()->getBytecodeCount() - loopStart + 2;
...
int Compiler::emitJump(uint8_t instruction) {
  ...
  emitByte(0xff);
  emitByte(0xff);
  return currentBytecode()->getBytecodeCount() - 2;
}

void Compiler::patchJump(int offset) {
  int jump = currentBytecode()->getBytecodeCount() - offset - 2;
  if (jump > std::numeric_limits<uint16_t>::max()) {
    reporter.error(previous, "Too much code to jump over.");
  }

  currentBytecode()->setBytecodeValue(offset, (jump >> 8) & 0xff);
  currentBytecode()->setBytecodeValue(offset+1, jump & 0xff);
}
...
void Compiler::endScope() {
  ...
  while (currentEnvironment->getLocalCount() > 0 &&
    ...
    if (local->isCaptured) {
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_CLOSE_UPVALUE));
    } else {
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
    }
...
void Compiler::printStatement() {
  ...
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_PRINT));

  // expression();

  // // Parse additional expressions separated by commas
  // while (match(TokenType::TOKEN_COMMA)) {
  //     // Emit bytecode to concatenate strings
  //     emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_ADD));
  //     expression();
  // }

  //   // Emit the print bytecode
  //   emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_PRINT));
...
void Compiler::and_() {
  int endJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP_IF_FALSE));
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
...
void Compiler::or_() {
  int elseJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP_IF_FALSE));
  int endJump = emitJump(static_cast<unsigned int>(Bytecode::OpCode::FOP_JUMP));
  patchJump(elseJump);
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_POP));
...
void Compiler::this_() {
  if (currentContractEnvironment == nullptr) {
    reporter.error(previous, "Cannot use 'this' outside of a contract.");
    return;
  }
...
void Compiler::super_() {
  if (currentContractEnvironment == nullptr) {
    reporter.error(previous, "Cannot use 'super' outside of a contract.");
  } else if (!(currentContractEnvironment->hasSupercontract)) {
    reporter.error(previous, "Cannot use 'super' in a contract with no supercontract.");
  }

  consume(TokenType::TOKEN_DOT, "Expect '.' after 'super'.");
  consume(TokenType::TOKEN_IDENTIFIER, "Expect super method name.");
  uint8_t name = identifierConstant(previous);
  namedVariable(syntheticToken("this"), false);
  if (match(TokenType::TOKEN_LEFT_PAREN)) {
    uint8_t argCount = argumentList();
    namedVariable(syntheticToken("super"), false);
    emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_SUPER_INVOKE),
              name);

    emitByte(argCount);
  } else {
    namedVariable(syntheticToken("super"), false);
    emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_GET_SUPER),
              name);
  }
}
...
void Compiler::call() {
  uint8_t argCount = argumentList();
  emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_CALL),
            argCount);
}
...
void Compiler::dot(bool canAssign) {
  ...
  if (canAssign && match(TokenType::TOKEN_EQUAL)) {
    expression();
    emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_SET_PROPERTY),
              name);
  } else if (match(TokenType::TOKEN_LEFT_PAREN)) {
    uint8_t argCount = argumentList();
    emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_INVOKE),
              name);

    emitByte(argCount);
  } else {
    emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_GET_PROPERTY),
              name);
  }
}
...
void Compiler::literal() {
  switch (previous.type) {
    case TokenType::TOKEN_FALSE:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_FALSE));
      break;
    case TokenType::TOKEN_NULL:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_NULL));
      break;
    case TokenType::TOKEN_TRUE:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_TRUE));
      break;
    default:
      return;
  }
...
void Compiler::namedVariable(const Token &name, bool canAssign) {
  ...
  if (arg != -1) {
    getOp = static_cast<unsigned int>(Bytecode::OpCode::FOP_GET_LOCAL);
    setOp = static_cast<unsigned int>(Bytecode::OpCode::FOP_SET_LOCAL);
  } else if ((arg = currentEnvironment->resolveUpvalue(name)) != -1) {
    getOp = static_cast<unsigned int>(Bytecode::OpCode::FOP_GET_UPVALUE);
    setOp = static_cast<unsigned int>(Bytecode::OpCode::FOP_SET_UPVALUE);
  } else {
    arg = identifierConstant(name);
    getOp = static_cast<unsigned int>(Bytecode::OpCode::FOP_GET_GLOBAL);
    setOp = static_cast<unsigned int>(Bytecode::OpCode::FOP_SET_GLOBAL);
  }
...
void Compiler::emitConstant(const Value &value) {
  emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_CONSTANT),
            makeConstant(value));
}

uint8_t Compiler::makeConstant(const Value &value) {
  auto constant = static_cast<unsigned int>(currentBytecode()->addConstant(value));
  if (constant > std::numeric_limits<uint8_t>::max()) {
    reporter.error(previous, "Too many constants in one Bytecode.");
    return 0;
  }
...
void Compiler::defineVariable(uint8_t global) {
  ...
  emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_DEFINE_GLOBAL),
            global);
}
...
void Compiler::unary() {
  ...
  switch (operatorType) {
    case TokenType::TOKEN_BANG:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_NOT));
      break;
    case TokenType::TOKEN_MINUS:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_NEGATE));
      break;
    default:
      return;
  }
...
void Compiler::binary() {
  ...
  switch (operatorType) {
    case TokenType::TOKEN_BANG_EQUAL:
      emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_EQUAL),
                static_cast<unsigned int>(Bytecode::OpCode::FOP_NOT));
      break;
    case TokenType::TOKEN_EQUAL_EQUAL:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_EQUAL));
      break;
    case TokenType::TOKEN_GREATER:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_GREATER));
      break;
    case TokenType::TOKEN_GREATER_EQUAL:
      emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_LESS),
                static_cast<unsigned int>(Bytecode::OpCode::FOP_NOT));
      break;
    case TokenType::TOKEN_LESS:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_LESS));
      break;
    case TokenType::TOKEN_LESS_EQUAL:
      emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_GREATER),
                static_cast<unsigned int>(Bytecode::OpCode::FOP_NOT));
      break;
    case TokenType::TOKEN_PLUS:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_ADD));
      break;
    case TokenType::TOKEN_MINUS:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_SUBTRACT));
      break;
    case TokenType::TOKEN_STAR:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_MULTIPLY));
      break;
    case TokenType::TOKEN_SLASH:
      emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_DIVIDE));
      break;
    default:
      return;
  }
...
void Compiler::emitByte(uint8_t byte) {
  currentBytecode()->appendByte(byte, previous.line);
}
...
std::shared_ptr<FunctionObject> Compiler::endCompiler() {
  ...
  if (!reporter.hadError()) {
    currentBytecode()->disassemble(function->getName() != nullptr ?
...
void Compiler::emitReturn() {
  if (currentEnvironment->getFunctionType() == FunctionObject::FunctionType::TYPE_INITIALIZER) {
    emitBytes(static_cast<unsigned int>(Bytecode::OpCode::FOP_GET_LOCAL), 0);
  } else {
    emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_NULL));
  }
  emitByte(static_cast<unsigned int>(Bytecode::OpCode::FOP_RETURN));
...
Bytecode* Compiler::currentBytecode() {
  return currentEnvironment->getFunction()->getBytecode();
}

void Compiler::reset() {
  currentTokenIndex = 0;
  currentContractEnvironment = nullptr;
  reporter.reset();
